...
import { LogisticsRemovePoints } from "@/components/shared/logistics-remove-points";
...
export function LogisticsPageContent({
  ...
  const showTabs = isAdmin || isSupervision;
    ...
    <div className="space-y-6">
      ...
      {showTabs && (
        <div className="flex gap-1 rounded-xl bg-white/[0.03] border border-white/8 p-1">
          ...
          {isSupervision && (
            <TabButton
              active={activeTab === "malus"}
              onClick={() => setActiveTab("malus")}
              variant="danger"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-4 h-4">
      ...
      {activeTab === "malus" && isSupervision && (
        <LogisticsRemovePoints utilisateurs={utilisateurs} />
      )}
...
function TabButton({
  ...
  const activeClass =
    variant === "danger"
      ? "bg-red-500/15 text-red-200 shadow-sm shadow-red-900/10 border border-red-500/20"
